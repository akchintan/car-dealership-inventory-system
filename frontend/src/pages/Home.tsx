...
import InventoryStatusChart, {
  InventoryStatusChartSkeleton,
} from '../components/charts/InventoryStatusChart'
...
function Home() {
  ...
  const availableCars = cars.filter(({ status }) => status.toLowerCase() === 'available').length
  const soldCars = cars.filter(({ status }) => status.toLowerCase() === 'sold').length
  const reservedCars = cars.filter(({ status }) => status.toLowerCase() === 'reserved').length
  const averagePrice =
    cars.length > 0 ? cars.reduce((total, car) => total + car.price, 0) / cars.length : 0
    ...
    <section style={pageStyle} aria-labelledby="dashboard-heading">
      ...
      <section aria-labelledby="quick-actions-heading" style={{ marginBottom: '40px' }}>
        <h2
          ...
          style={{ margin: '0 0 16px', color: '#172033', fontSize: '1.2rem' }}
        >
          Quick actions
        </h2>
        <div
        ...
        >
          <Card>
            ...
            <p style={{ margin: '8px 0 20px', color: '#667085', lineHeight: 1.5 }}>
          ...
          <Card>
            ...
            <p style={{ margin: '8px 0 20px', color: '#667085', lineHeight: 1.5 }}>
              Review, update, and manage your listed vehicles.
            </p>
      ...
      <section aria-labelledby="inventory-stats-heading" aria-busy={isLoading}>
        <h2
          id="inventory-stats-heading"
          style={{ margin: '0 0 16px', color: '#172033', fontSize: '1.2rem' }}
        >
          Inventory summary
        </h2>
      ...
      <section
        aria-labelledby="inventory-distribution-heading"
        aria-busy={isLoading}
        style={{ marginTop: '40px' }}
      >
        <h2
          id="inventory-distribution-heading"
          style={{ margin: '0 0 16px', color: '#172033', fontSize: '1.2rem' }}
        >
